  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4 h-full">
        <div
          className="m-auto h-[80%] p-4 cursor-pointer bg-white "
        ...
        <div className=" h-[20%]">
          <ul style={thumbnailsstyle} className="">
            {medias?.map((thumbnail, index) => {
              ...
              if (!thumbnail.isMain) return;
              // if(thumbnail.index === thumbnail.length) return alert("3")
              const style =
                index === thumbnail.length - 1 ? { padding: "100rem" } : "";
              ...
              return (
                <li key={index} className="  cursor-pointer last:blur-sm">
                  {thumbnail.type === "video" ? (
                    ...
                    >
                      <div
                        className=" relative rounded-lg border border-solid blur-sm border-gray-200 bg-white overflow-hidden p-2 hover:opacity-80"
                        onClick={() => setShowPopUp(true)}
                      >
                        <Image
                      ...
                      >
                        <div
                          className="absolute top-0 right-0 w-full h-full flex justify-center items-center filter blur-lg"
                          onClick={() => setShowPopUp(true)}
                        ></div>
                        <div
                          className="absolute top-1/2 right-[20%] -translate-x-[50%] -translate-y-[50%]"
                          onClick={() => setShowPopUp(true)}
                        >
                          <CgPlayButtonR
